...
async def handle_error(client, message):
    await client.send_message(message.chat.id, "**❌ Looks Like I Am Not Admin Here Add Me As Admin First**", parse_mode=ParseMode.MARKDOWN)
...
def setup_ban_handlers(app):
    ...
    async def handle_kick(client, message: Message):
        ...
        if user_id and not await is_admin(app, user_id, chat_id):
            await message.reply_text("**🚫 Hey Gay 🏳️‍🌈 This Is Not For You This Only For Males👱‍♂️**", parse_mode=ParseMode.MARKDOWN)
            return
    ...
    async def handle_ban(client, message):
        ...
        if user_id and not await is_admin(app, user_id, chat_id):
            await client.send_message(message.chat.id, "**🚫 Hey Gay 🏳️‍🌈 This Is Not For You This Only For Males👱‍♂️**", parse_mode=ParseMode.MARKDOWN)
            return
    ...
    async def handle_unban(client, message):
        ...
        try:
            if message.reply_to_message:
                ...
                user_info = await app.get_users(target_user)
                username = user_info.username if user_info.username else user_info.first_name
                await client.send_message(message.chat.id, f"**User {username} has been unbanned.** ✅", parse_mode=ParseMode.MARKDOWN)
            else:
                for target_user in target_users:
                    user_info = await app.get_users(target_user)
                    target_user_id = user_info.id
    ...
    async def callback_unban(client, callback_query):
        ...
        try:
            ...
            user_info = await app.get_users(target_user)
            username = user_info.username if user_info.username else user_info.first_name
            await callback_query.answer(f"User {username} has been unbanned.")
            await callback_query.message.edit_text(f"**User {username} has been unbanned.** ✅", parse_mode=ParseMode.MARKDOWN)
        except Exception as e:
    ...
    async def handle_unmute(client, message):
        ...
        try:
            if message.reply_to_message:
                await app.restrict_chat_member(chat_id, target_user, permissions=ChatPermissions(
                ...
                user_info = await app.get_users(target_user)
                username = user_info.username if user_info.username else user_info.first_name
                await client.send_message(message.chat.id, f"**User {username} has been unmuted.** ✅", parse_mode=ParseMode.MARKDOWN)
            else:
                for target_user in target_users:
                    user_info = await app.get_users(target_user)
                    target_user_id = user_info.id
                    await app.restrict_chat_member(chat_id, target_user_id, permissions=ChatPermissions(
                        can_send_messages=True,
                        can_send_media_messages=True,
                        can_send_polls=True,
                        can_send_other_messages=True,
    ...
    async def callback_unmute(client, callback_query):
        ...
        try:
            ...
            user_info = await app.get_users(target_user)
            username = user_info.username if user_info.username else user_info.first_name
            await callback_query.answer(f"User {username} has been unmuted.")
            await callback_query.message.edit_text(f"**User {username} has been unmuted.** ✅", parse_mode=ParseMode.MARKDOWN)
        except Exception as e:
            logger.error(f"Error in unmute callback: {e}")
            await callback_query.answer("🚫 Hey Gay 🏳️‍🌈 This Is Not For You This Only For Males👱‍♂️", show_alert=True)