...
class TikTokDownloader:
    ...
    async def download_media(self, url: str, downloading_message: Message) -> Optional[dict]:
        ...
        api_url = f"https://tik-tok-downloader.vercel.app/dl?url={url}"
        try:
...
def setup_tt_handler(app: Client):
    ...
    @app.on_message(
        filters.regex(rf"^{command_prefix_regex}(tik|tiktok)(\s+https?://(vm\.tiktok\.com|www\.tiktok\.com)/\S+)?$") &
        (filters.private | filters.group)
    )
    async def tik_handler(client: Client, message: Message):
